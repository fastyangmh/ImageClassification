...
def pytorch_mnist_to_png(root):
    ...
    for stage in ['train', 'test']:
        target_path = os.path.join(dirpath, 'images/{}'.format(stage))
        os.makedirs(target_path, exist_ok=True)
        images = mnist.read_image_file(path=os.path.join(
            dirpath, 'raw/{}'.format(files[stage]['images'])))
        labels = mnist.read_label_file(path=os.path.join(
            dirpath, 'raw/{}'.format(files[stage]['labels'])))
...
def pytorch_cifar10_to_png(root):
    for dirpath, _, filenames in os.path.walk(root):
        ...
        for file in files[stage]:
            with open(
                    os.path.join(dirpath,
                                 'cifar-10-batches-py/{}'.format(file)),
                    'rb') as f:
